...
public class BookshelfServletImpl extends HttpServlet {
	...
	private void htmlCategories(PrintWriter printWriter)
	...
	private void htmlLoginForm(PrintWriter printWriter, String errorMessage) {
		...
		printWriter.println("<input type=\"hidden\" name=\"" + PARAM_OP
				+ "\" value=\"" + OP_LOGIN + "\">");
		printWriter.println("User: <input type=\"text\" name=\"" + PARAM_USER
				+ "\"><br />");
		printWriter.println("Pass: <input type=\"password\" name=\""
				+ PARAM_PASS + "\"><br />");
	...
	private void htmlTop(PrintWriter printWriter) {
		...
		printWriter.println("<td><a href=\"" + categoriesUrl()
				+ "\">Browse by category</a></td>");
		printWriter.println("<td><a href=\"" + searchByAuthorUrl(null)
				+ "\">Search by author</a></td>");
		printWriter.println("<td><a href=\"" + addBookFormUrl()
				+ "\">Add book</a></td>");
	...
	private String browseByCategoryUrl(String category) {
		return "?" + PARAM_OP + "=" + OP_BYCATEGORY + "&" + PARAM_CATEGORY
				+ "=" + category;
	}
	...
	private void htmlByCategory(PrintWriter printWriter, String category)
		...
		if (category != null) {
			...
			for (String isbn : this.service.searchBooksByCategory(
					this.sessionId, category)) {
				printWriter.println("<tr><td>" + isbn + "</td>");
				Book book;
				try {
					book = this.service.getBook(this.sessionId, isbn);
					printWriter.println("<td>" + book.getRating() + "</td><td>"
							+ book.getTitle() + "</td><td>" + book.getAuthor()
							+ "</td>");
				} catch (BookNotFoundException e) {
					printWriter.println("<td>-</td><td>" + e.getMessage()
							+ "</td><td>-</td>");
				}
	...
	private String searchByAuthorUrl(String author) {
		if (author == null) {
			...
			return "?" + PARAM_OP + "=" + OP_BYAUTHOR + "&" + PARAM_AUTHOR
					+ "=" + author;
		}
	}
	...
	private void htmlByAuthor(PrintWriter printWriter, String author)
		...
		if (author != null) {
			...
			printWriter.println("<table border=\"1\">");
			printWriter
					.println("<tr><th>ISBN</th><th>Rating</th><th>Title</th><th>Category</th></tr>");
			for (String isbn : this.service.searchBooksByAuthor(this.sessionId,
					author)) {
				printWriter.println("<tr><td>" + isbn + "</td>");
				Book book;
				try {
					book = this.service.getBook(this.sessionId, isbn);
					printWriter.println("<td>" + book.getRating() + "</td><td>"
							+ book.getTitle() + "</td><td>" + book.getRating()
							+ "</td>");
				} catch (BookNotFoundException e) {
					printWriter.println("<td>-</td><td>-</td><td>"
							+ e.getMessage() + "</td>");
				}
	...
	private void htmlAddBookForm(PrintWriter printWriter) {
		...
		printWriter.println("<tr><td><input type=\"hidden\" name=\"" + PARAM_OP
				+ "\" value=\"" + OP_ADDBOOK + "\">");
		printWriter
				.println("<tr><td>ISBN:</td><td><input type=\"text\" name=\""
						+ PARAM_ISBN + "\"></td>");
		printWriter
				.println("<tr><td>Author:</td><td><input type=\"text\" name=\""
						+ PARAM_AUTHOR + "\"></td>");
		printWriter
				.println("<tr><td>Title:</td><td><input type=\"text\" name=\""
						+ PARAM_TITLE + "\"></td>");
		printWriter
				.println("<tr><td>Category:</td><td><input type=\"text\" name=\""
						+ PARAM_CATEGORY + "\"></td>");
		printWriter
				.println("<tr><td>Rating:</td><td><input type=\"text\" name=\""
						+ PARAM_RATING + "\"></td>");

		printWriter
				.println("<tr><td colspan=\"2\"><input type=\"submit\" value=\"Add\">");
	...
	private void doAddBook(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			this.service.addBook(this.sessionId, isbn, title, author, category,
					rating);
		} catch (Exception e) {
			resp.getWriter().println(e.getMessage());
			return;
